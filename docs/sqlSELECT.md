...
### FILL
Defines filling strategy for missing data in aggregation queries.
...
syntaxes for [text](where.md#symbol-and-string), [numeric](where.md#numeric), or [timestamp](where.md#timestamp-and-date) filters.
:::
...
### SAMPLE BY 
Aggregates time-series data into homogeneous time-chunks. For example daily average, monthly maximum etc.
This function requires a [designated timestamp](designatedTimestamp.md).
...
Dynamically creates a [designated timestamp](designatedTimestamp.md) on the output of a query. This allows to perform timestamp operations 
like [SAMPLE BY](#sample-by) or [LATEST BY](#latest-by) on tables which originally do not have a designated timestamp.
